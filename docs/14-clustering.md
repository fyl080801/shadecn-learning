...
| 位置 | 状态 | 多副本下会怎样 |
|---|---|---|
| `collab/persistence.ts` | 审计日志的 `nextSeq` | 两个进程各自 +1，直接撞 `@@unique([flowId, seq])` |
...
**审计只由收到客户端消息的那个实例记**（`shouldRecordUpdate`）。同一条更新会经 pub/sub
转发到每个持有该房间的实例，每个都触发一次 `onChange` —— 照单全收的话审计表就成了实例数的倍数
（实测两个实例、11 次改动写出 22 行），而且转发来的那条手里没有作者的 `context`，
`actorId` 只能是 null。判据是 `transactionOrigin.source !== 'redis'`；认不出形状时按「记」处理，
宁可多一条也不能把作者那条丢了。

**`FlowOperation.serverTs` 在收到更新的那一刻就打**，不是等排到写队列、取完号再打。
队列前面可能压着几次写，多副本下取号还夹着一次 Redis 往返 —— 那之后再读时钟，
记下来的就成了「什么时候轮到它写」。实测过：并发时会出现 seq 更大、`serverTs` 反而更小的行。
跨实例的时钟偏差没法在应用层解决，**排序请始终用 `seq`**，那个是全局单调的。
...
- `persistence.ts` 的写队列：seq 改成 `INCR` 之后跨实例不再冲突，只需本实例内保持顺序。
...
- **Redis 只是缓存，不放业务数据**。里面全是可重建的东西：计数器丢了从库里的 `max(seq)` 重新播种，
...
- [x] **审计 seq 跨实例唯一且连续**（11 条，1..11）—— 这是单副本时代会撞 `@@unique([flowId, seq])` 的那个点。
- [x] **审计流可回放**：按 seq 把 11 条增量重放到空文档，结果和库里的 `ydoc` 快照一致（10 节点 / 1 连线）。
      这一条专门用来证明「不记 redis 转发的那条」没有删漏。
- [x] `serverTs` 在 seq 顺序上单调（修复前有乱序行）。
...
- 多副本下 `FlowOperation` 的写入量会随实例数不变（还是每次改动一条），但**并发写同一张表**的压力上去了，
  PostgreSQL 那边要不要给 `(flowId, seq)` 之外再加索引，等有真实流量再看。
