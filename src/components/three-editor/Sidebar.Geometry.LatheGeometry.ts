...
import { UIDiv, UIRow, UIText, UIInteger, UINumber } from "./libs/ui"
import { UIPoints2 } from "./libs/ui.three"
...
function GeometryParametersPanel(editor, object) {
  ...
  const phiStart = new UINumber(
    parameters.phiStart * THREE.MathUtils.RAD2DEG
  ).onChange(update)
  ...
  const phiLength = new UINumber(
    parameters.phiLength * THREE.MathUtils.RAD2DEG
  ).onChange(update)
