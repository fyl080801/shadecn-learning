...
- `server/config.ts` — `rootDir` / `outputDir` / `staticDir` / `isDev` / `port` / `host` / `isApiPath()` / `dataDir` / `databaseUrl` / `ensureDatabaseDir()` / `appOrigin` / `authConfig` / `authEnabled` / `assertAuthConfig()`. `isDev` is `NODE_ENV !== 'production'`; `staticDir` defaults to `output/public` (next to the bundle when bundled) and `STATIC_DIR` overrides it
