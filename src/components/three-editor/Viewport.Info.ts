// @ts-nocheck
import { UIPanel, UIBreak, UIText } from "./libs/ui"
...
function ViewportInfo(editor) {
  ...
  const objectsText = new UIText("0")
    .setTextAlign("right")
    .setWidth("60px")
    .setMarginRight("6px")
  const verticesText = new UIText("0")
    .setTextAlign("right")
    .setWidth("60px")
    .setMarginRight("6px")
  ...
  const trianglesUnitText = new UIText(
    strings.getKey("viewport/info/triangles")
  )
  const samplesUnitText = new UIText(
    strings.getKey("viewport/info/samples")
  ).setHidden(true)
  ...
  container.add(
    frametimeText,
    new UIText(strings.getKey("viewport/info/rendertime")),
    new UIBreak()
  )
  ...
  function update() {
    ...
    const objectsStringKey =
      pluralRules.select(objects) === "one"
        ? "viewport/info/object"
        : "viewport/info/objects"
    objectsUnitText.setValue(strings.getKey(objectsStringKey))

    const verticesStringKey =
      pluralRules.select(vertices) === "one"
        ? "viewport/info/vertex"
        : "viewport/info/vertices"
    verticesUnitText.setValue(strings.getKey(verticesStringKey))

    const trianglesStringKey =
      pluralRules.select(triangles) === "one"
  ...
  editor.signals.pathTracerUpdated.add(function (samples) {
    ...
    const samplesStringKey =
      pluralRules.select(samples) === "one"
        ? "viewport/info/sample"
        : "viewport/info/samples"
