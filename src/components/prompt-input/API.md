...
| `containerClass`    | `string`         | `undefined`                       | ❌   | 覆盖编辑区容器类名；缺省应用默认浅色主题样式                       |
| `wrapperClass`      | `string`         | `undefined`                       | ❌   | 覆盖外层 wrapper 类名（边框、背景、圆角）                          |
| `disabled`          | `boolean`        | `false`                           | ❌   | 只读模式（contenteditable="false"）                                |
| `maxLength`         | `number \| null` | `null`                            | ❌   | 最大可见字符数（剔除 \n 和零宽空格）；null = 不限制、不渲染 footer |
| `showCounter`       | `boolean`        | `false`                           | ❌   | 在 footer 显示字符计数器（需设置 maxLength）                       |
| `showClear`         | `boolean`        | `false`                           | ❌   | 在 footer 显示清空按钮                                             |
| `deferFocusOnClick` | `boolean`        | `false`                           | ❌   | 挂载时不自动聚焦 contenteditable，避免画布场景 Delete 键失效       |
...
| `exceed-limit`      | `currentCount: number, limit: number` | 输入/粘贴/提交超过 maxLength 时触发         |
...
| `#footer-action`         | —                                                                                           | footer 操作区（需设置 maxLength 才渲染 footer）                |
...
| `editableEl`            | `Ref<HTMLElement \| null>`           | contenteditable 根元素引用（非 wrapper）                                |
| `charCount`             | `ComputedRef<number>`                | 当前可见字符数                                                          |
...
const mention = definePlugin({
  ...
  trigger: { key: "@" },
  parse: (text) =>
    splitByRegex(text, /@\[([^\]]+)\]\(([^)]+)\)/g, (m) => ({
      kind: "node",
      type: "mention",
      data: { id: m[2], label: m[1] }
    })),
  serialize: (node) => {
