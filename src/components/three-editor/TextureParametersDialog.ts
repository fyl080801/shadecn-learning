...
import { renderToCanvas } from "./libs/ui.three"
...
class TextureParametersDialog {
  constructor(editor, texture) {
    ...
    previewWrapper.add(
      createGroupHeading(this.strings.getKey("dialog/texture/group/preview"))
    )
    ...
    form.add(
      createGroupHeading(this.strings.getKey("dialog/texture/group/mapping"))
    )

    this.mapping = new UISelect()
      .setOptions({
        [THREE.UVMapping]: "UV",
    ...
    form.add(
      createRow(this.strings.getKey("dialog/texture/mapping"), this.mapping)
    )
    ...
    this.wrapS = new UISelect()
      .setOptions(wrapOptions)
      .setValue(texture.wrapS)
      .onChange(updatePreview)
    form.add(createRow(this.strings.getKey("dialog/texture/wrapS"), this.wrapS))

    this.wrapT = new UISelect()
      .setOptions(wrapOptions)
      .setValue(texture.wrapT)
      .onChange(updatePreview)
    ...
    form.add(
      createGroupHeading(this.strings.getKey("dialog/texture/group/filtering"))
    )

    this.minFilter = new UISelect()
      .setOptions({
      ...
      .setValue(texture.minFilter)
      .onChange(updatePreview)
    form.add(
      createRow(this.strings.getKey("dialog/texture/minFilter"), this.minFilter)
    )

    this.magFilter = new UISelect()
      .setOptions({
        [THREE.NearestFilter]: "Nearest",
        [THREE.LinearFilter]: "Linear"
      })
      .setValue(texture.magFilter)
      .onChange(updatePreview)
    form.add(
      createRow(this.strings.getKey("dialog/texture/magFilter"), this.magFilter)
    )

    this.anisotropy = new UINumber(texture.anisotropy)
      .setPrecision(0)
      .setRange(1, 16)
      .setNudge(1)
      .setStep(1)
    ...
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/anisotropy"),
        this.anisotropy
      )
    )
    ...
    form.add(
      createGroupHeading(this.strings.getKey("dialog/texture/group/transform"))
    )

    this.offsetX = new UINumber(texture.offset.x)
      .setWidth("60px")
      .onChange(updatePreview)
    this.offsetY = new UINumber(texture.offset.y)
      .setWidth("60px")
      .onChange(updatePreview)
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/offset"),
        this.offsetX,
        this.offsetY
      )
    )

    this.repeatX = new UINumber(texture.repeat.x)
      .setWidth("60px")
      .onChange(updatePreview)
    this.repeatY = new UINumber(texture.repeat.y)
      .setWidth("60px")
      .onChange(updatePreview)
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/repeat"),
        this.repeatX,
        this.repeatY
      )
    )

    this.centerX = new UINumber(texture.center.x)
      .setWidth("60px")
      .onChange(updatePreview)
    this.centerY = new UINumber(texture.center.y)
      .setWidth("60px")
      .onChange(updatePreview)
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/center"),
        this.centerX,
        this.centerY
      )
    )

    this.rotation = new UINumber(texture.rotation * THREE.MathUtils.RAD2DEG)
      .setStep(10)
      .setNudge(0.1)
      .setUnit("°")
      .setWidth("60px")
      .onChange(updatePreview)
    form.add(
      createRow(this.strings.getKey("dialog/texture/rotation"), this.rotation)
    )

    // Color group

    form.add(
      createGroupHeading(this.strings.getKey("dialog/texture/group/color"))
    )

    this.premultiplyAlpha = new UICheckbox(texture.premultiplyAlpha).onChange(
      updatePreview
    )
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/premultiplyAlpha"),
        this.premultiplyAlpha
      )
    )

    this.colorSpace = new UISelect()
      .setOptions({
        [THREE.NoColorSpace]: "No Color Space",
        [THREE.SRGBColorSpace]: "sRGB",
        [THREE.LinearSRGBColorSpace]: "Linear sRGB"
      })
      .setValue(texture.colorSpace)
      .onChange(updatePreview)
    form.add(
      createRow(
        this.strings.getKey("dialog/texture/colorSpace"),
        this.colorSpace
      )
    )
