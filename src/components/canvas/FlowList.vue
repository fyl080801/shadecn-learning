 ...
 *
 * 两处在用，而且**必须是同一个组件**：项目主页的「画布」Tab，和个人画布页
 * `/personal`（REQ-SOLO §4.8）。个人空间在接口层面就是个项目，所以这里
 * 一个 `projectId` 就够了 —— 组件不需要知道自己列的是哪一种。
 *
 * 反过来，**界面上这两种画布是分开的**：个人画布有自己的侧栏入口、自己的页面，
 * 不和项目并列在一个 Tab 栏里。这个组件是实现上的复用，不是界面上的合并。
 *
 * **不碰路由是默认行为**（`syncQuery`）。两处调用现在都开着它 —— 一个页面里
 * 只有一个列表，页码和关键字写进 URL 是对的。这个开关是并列 Tab 时期留下的：
 * 那时两个列表共用一份 query，「项目搜到第 3 页」跟到个人画布那边就落在空页上，
 * 只能让其中一个独占 URL。拆成两页之后没有这个冲突了，开关先留着。
 */
