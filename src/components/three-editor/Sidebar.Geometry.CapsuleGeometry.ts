...
import { UIDiv, UIRow, UIText, UINumber, UIInteger } from "./libs/ui"
...
function GeometryParametersPanel(editor, object) {
  ...
  heightRow.add(
    new UIText(
      strings.getKey("sidebar/geometry/capsule_geometry/height")
    ).setClass("Label")
  )
  ...
  const capSegments = new UIInteger(parameters.capSegments)
    .setRange(1, Infinity)
    .onChange(update)
  ...
  heightSegmentsRow.add(
    new UIText(
      strings.getKey("sidebar/geometry/capsule_geometry/heightseg")
    ).setClass("Label")
  )
