...
export interface FlowTransport {
   ...
   * **服务端收到了，但存不进库**。
   *
   * 和 `pending`（本地还没发出去）是两回事，也和断线是两回事 —— 连接好好的、
   * 内容也同步给了同房间的人，只是服务器那头写库失败了，字节悬在它的内存里。
   * 不把这件事说出来，界面就会一直写着「已同步」，而用户照着继续画。
   *
   * 只有协同画布会为真：个人画布的推送**就是**写库，成不成 `pending` 已经说清楚了。
   * 服务器恢复之后会再广播一次，这个值自己变回 false。
   */
