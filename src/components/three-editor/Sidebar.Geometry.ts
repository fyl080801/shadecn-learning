...
import { SidebarGeometryBufferGeometry } from "./Sidebar.Geometry.BufferGeometry"
import { SidebarGeometryModifiers } from "./Sidebar.Geometry.Modifiers"
...
function SidebarGeometry(editor) {
  ...
  geometryTypeRow.add(
    new UIText(strings.getKey("sidebar/geometry/type")).setClass("Label")
  )
  ...
  geometryUUIDRow.add(
    new UIText(strings.getKey("sidebar/geometry/uuid")).setClass("Label")
  )
  ...
  geometryNameRow.add(
    new UIText(strings.getKey("sidebar/geometry/name")).setClass("Label")
  )
  ...
  const geometryUserData = new UITextArea()
    ...
    .onChange(function () {
      try {
        ...
        if (
          JSON.stringify(editor.selected.geometry.userData) !=
  ...
  vertexNormalsButton.onClick(function () {
    ...
    if (editor.helpers[object.id] === undefined) {
      editor.addHelper(
        object,
        new VertexNormalsHelper(object, vertexNormalsSize.getValue())
      )
    } else {
  ...
  const vertexNormalsSize = new UINumber(1)
    .setWidth("30px")
    .setMarginLeft("7px")
    .setRange(0, Infinity)
    .onChange(function () {
  ...
  exportJson.onClick(function () {
    ...
    editor.utils.save(
      new Blob([output]),
      `${geometryName.getValue() || "geometry"}.json`
    )
  })
  ...
  morphContainer.add(
    new UIText(strings.getKey("sidebar/geometry/morph")).setTextTransform(
      "uppercase"
    )
  )
      ...
      if (currentGeometryType !== geometry.type) {
        ...
        if (
          ...
          const { GeometryParametersPanel } = await import(
            `./Sidebar.Geometry.${geometry.type}.ts`
          )
