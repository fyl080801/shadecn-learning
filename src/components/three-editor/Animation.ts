// @ts-nocheck
import { UIPanel, UIText, UIButton, UINumber } from "./libs/ui"
...
function Animation(editor) {
  ...
  const pauseIcon =
    '<svg width="12" height="12" viewBox="0 0 12 12"><path d="M2 1h3v10H2zM7 1h3v10H7z" fill="currentColor"/></svg>'
  const stopIcon =
  ...
  function updateTime() {
    if (currentAction && currentClip) {
      ...
      const timelineWidth = rect.width - labelWidth
      const playheadX =
        labelWidth + (time / currentClip.duration) * timelineWidth
