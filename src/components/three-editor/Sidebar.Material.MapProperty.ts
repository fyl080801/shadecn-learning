...
import { UIButton, UICheckbox, UIDiv, UINumber, UIRow, UIText } from "./libs/ui"
...
function SidebarMaterialMapProperty(editor, property, name) {
  ...
  const settings = new UIButton("⚙")
    .setClass("TextureSettingsButton")
    .setMarginRight("4px")
  ...
  async function onSettingsClick() {
    ...
    try {
      ...
      editor.execute(
        new SetTextureParametersCommand(editor, texture, parameters)
      )
    } catch (e) {
      // Edit cancelled
    }
