...
import { UINumber, UIPanel, UIRow, UISelect, UIText } from "./libs/ui"
...
function SidebarProjectRenderer(editor) {
  ...
  cameraRow.add(
    new UIText(strings.getKey("sidebar/project/camera")).setClass("Label")
  )

  const cameraTypeSelect = new UISelect()
    .setOptions({
  ...
  rendererRow.add(
    new UIText(strings.getKey("sidebar/project/renderer")).setClass("Label")
  )
